...
contract TokenVesting is Ownable, ReentrancyGuard {
    ...
    function lockTokens(uint256 _amount) external {
        ...
        // Ensure the user has approved the token transfer before calling lockTokens
    ...
    function whiteListStakeholders(
        ...
        require(organizations[msg.sender].stakeholder[msg.sender].stakeholderAddress == address(0), "Already Whitelisted");
        ...
        organizations[msg.sender].stakeholder[_Stakeholder].vestingSchedule = VestingSchedule(_startTime, _endTime, _amount);
    ...
    function isWhiteListed(address _stakeholderAddress) external view onlyRegisteredOrg returns (bool) {
        require(organizations[msg.sender].stakeholder[_stakeholderAddress].stakeholderAddress != address(0), "Not a stakeholder");
        return organizations[msg.sender].stakeholder[_stakeholderAddress].isWhitelisted;
    }

    function claimTokens(address org) external {
        require(organizations[org].orgAddress != address(0), "Not a Registered Organization");
        ...
        require(organizations[org].stakeholder[msg.sender].stakeholderAddress != address(0), "You are not Whitelisted for the claim");
        require(!organizations[org].stakeholder[msg.sender].isClaimed, "You have already claimed the amount");
        require(organizations[org].stakeholder[msg.sender].isWhitelisted, "You are not Whitelisted for the claim");
